...
app.post("*", async (c) => {
  ...
  // Resolve auth: JWT (preferred) or static access key (backward compat)
  ...
  // --- MCP Server (per-request, closes over supabase & userId) ---
  ...
  server.registerTool(
    ...
    async ({ content, due_date, tags, thought_id, blocked_by, unblocks, recurrence }) => {
      try {
        const row: Record<string, unknown> = { content, user_id: userId };
  ...
  server.registerTool(
    ...
    async ({ days, limit }) => {
      try {
        const items = await detectStaleLoops(supabase, getEmbedding, { days, limit, userId });
        const output = formatStaleLoopsOutput(items, days);
